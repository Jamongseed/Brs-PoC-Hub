...
(function injectThirdpartySdkOnce() {
  try {
    const cfg = window.__POC_G__ || {};
    const sdkUrl = String(cfg.sdkUrl || "").trim();
    if (!sdkUrl) return;
    const exists = Array.from(document.scripts).some(s => s && s.src === sdkUrl);
    if (exists) return;
...
try {
  const cfg = window.__POC_G__ || {};
  sdkOriginEl.textContent = cfg.attackerOrigin || "-";
} catch (_) {}
